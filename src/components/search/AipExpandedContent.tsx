...
export default function AipExpandedContent({ aipId, showHeaderButtons = true }: Props) {
  ...
  async function downloadPackage() {
    ...
    try {
      ...
      const config = await loadConfig();
      const sourceText = config.downloadConfig?.sourceText ?? '';
      const downloadFn = await loadPackageUtil();
      await downloadFn(title, aipId, filteredMeta, allFiles, sourceText);
    } catch (err) {
      ...
      {showHeaderButtons && (
        <div className="expanded-content__files-actions">
          <button
            ...
            aria-label="Öppna och ladda ner fil"
          >
            {isDownloading ? (
